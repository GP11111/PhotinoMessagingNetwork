...
//
//Socket listener class for DNR to DNR messaging
//
class DNRSocketListener extends Thread
{
	...
	public DNRSocketListener(DatagramSocket socketIn, DatagramSocket socketOut, int port) 
	{
		...
		int portIn = 0;
		byte[] receiveDataIn = new byte[PhotinoDNR.udppacketsize];
		String contentIn = "";
		...
		{ 
		  ...
		  try
		  {
			  receiveDataIn = new byte[PhotinoDNR.udppacketsize];
		      ...
		      if ( !PhotinoDNR.blacklistip.containsKey(IPAddressIn.getHostAddress()) )
		      {
		    	  //
